(ns medisoft.frontend.ui.input-time
...
(defn- on-defocus
  ...
  (let [time (- (text->time @text-model) 0) ; TODO: timezone hack
...
(defn input-time
  ...
  (let [deref-model    (deref-or-value model)
        text-model     (reagent/atom (time->text (+ 200 deref-model))) ; TODO: timezone hack
