...
(defmethod form-field-component :input [name' {:keys [data errors labelize] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
              ...
              (when-let [errors (get-in @errors name')]
                [rc-core/label :label (str/join ", " errors)
                 :style {:padding-top "10px"}
                 :class "text-danger"])]])

(defmethod form-field-component :text-area [name' {:keys [data errors labelize] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
...
(defmethod form-field-component :password [name' {:keys [data errors labelize] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
              [rc-core/input-text :model (or (get-in @data name') "")
                                  :on-change   #(swap! data assoc-in name' %)
...
(defmethod form-field-component :select [name' {:keys [data errors choices model on-change labelize label-fn selected-label-fn] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
...
(defmethod form-field-component :date [name' {:keys [data errors labelize model on-change disabled-dates marked-dates] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
...
(defmethod form-field-component :time [name' {:keys [data errors labelize model on-change] :as opts}]
  [rc-core/v-box :class    "form-group"
   :children [[:label #_{:for "login-form-login"} (or (:label opts) (labelize name'))]
...
(defmethod form-field-component :date-time [name' {:keys [data errors labelize model on-change disabled-dates marked-dates] :as opts}]
...
(defmethod form-field-component :checkbox [name' {:keys [data errors labelize] :as opts}]
...
(defn make-form-field-maker [data errors &[{:as form-opts}]]
  (let [l18n-scopes (or (:l18n-scopes form-opts) [])
        ; _ (log/error "L18N SCOPES" form-opts l18n-scopes)
        labelize-fn (fn [field-name]
                      (or
                        (some (fn [scope]
                                (let [field-lead (butlast field-name)
                                      scope (str/join "." (filter not-empty (flatten [(name scope) field-lead])))
                                      field-name (name (last field-name))
                                      tl-key (keyword (name scope) field-name)]
                                  (log/debug "CHECKING TL KEY" tl-key)
                                  (not-empty (l18n/t tl-key))))
                              l18n-scopes)
                        (do
                          (log/debug "NO TRANSLATION FOUND FOR" field-name "IN SCOPES" l18n-scopes)
                          (labelize field-name))))
        default-opts {:type :input}] ;atom atom]
    (fn [name' opts]
      (let [new-name (mapv keyword (flatten [name']))]
        (form-field-component new-name (merge default-opts opts {:data data :errors errors :labelize labelize-fn}))))))
